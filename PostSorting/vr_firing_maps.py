...
def calculate_firing_rate(spike_data, spatial_data):
    ...
    for cluster in range(len(spike_data)):
        ...
        for t in range(1,int(number_of_trials)):
            try:
                trial_indices_b=np.where(trials_b == t)[0]
                trial_locations_b = np.take(locations_b, trial_indices_b)
                trial_indices_nb=np.where(trials_nb == t)[0]
                trial_locations_nb = np.take(locations_nb, trial_indices_nb)
                for loc in range(int(number_of_bins)):
                    ...
                    firing_rate_map = firing_rate_map.append( {"trial_number": int(t), "bin_count": int(loc), "b_spike_number":  len(spikes_in_bin_b), "nb_spike_number":  len(spikes_in_bin_nb)}, ignore_index=True)
