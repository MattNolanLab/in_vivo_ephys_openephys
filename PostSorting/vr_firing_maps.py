...
def average_spikes_over_trials(firing_rate_map,number_of_bins):
    ...
    avg_spikes_across_trials_b = PostSorting.vr_spatial_data.get_rolling_sum(avg_spikes_across_trials_b, 10)
    avg_spikes_across_trials_nb = PostSorting.vr_spatial_data.get_rolling_sum(avg_spikes_across_trials_nb, 10)
    avg_spikes_across_trials_p = PostSorting.vr_spatial_data.get_rolling_sum(avg_spikes_across_trials_p, 10)
...
def calculate_firing_rate(spike_data, spatial_data):
    ...
    firing_rate_map = pd.DataFrame(columns=['trial_number', 'bin_count', 'b_spike_number', 'nb_spike_number','p_spike_number','dwell_time_ms', 'normalised_spikes'])
    ...
    firing_rate_map['normalised_spikes'] = (spatial_data['b_spike_number'] / firing_rate_map['dwell_time'])
