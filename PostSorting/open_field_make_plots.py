...
def plot_hd(spatial_firing, position_data, figure_folder_path):
    ...
    for cluster_index, cluster_id in enumerate(spatial_firing.cluster_id):
        ...
        plt.savefig(figure_folder_path + cluster_df['session_id'].iloc[0] + '_hd_map_' + str(cluster_id) + '.png', dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close()
...
def plot_polar_head_direction_histogram(hd_hist, spatial_firing, figure_folder_path):
    ...
    for cluster_index, cluster_id in enumerate(spatial_firing.cluster_id):
        ...
        plt.savefig(figure_folder_path + cluster_df['session_id'].iloc[0] + '_hd_polar_' + str(cluster_id) + '.png', dpi=300, bbox_inches="tight")
        plt.close()
...
def plot_rate_map_autocorrelogram(spatial_firing, figure_folder_path):
    ...
    for cluster_index, cluster_id in enumerate(spatial_firing.cluster_id):
        ...
        if rate_map_autocorr.size:
            ...
            plt.savefig(figure_folder_path + cluster_df['session_id'].iloc[0] + '_rate_map_autocorrelogram_' + str(cluster_id) + '.png', dpi=300, bbox_inches="tight")
...
def save_field_polar_plot(figure_folder_path, hd_hist_session, hd_hist_cluster, cluster_id, spatial_firing, colors, field_id, name):
    cluster_df = spatial_firing[(spatial_firing.cluster_id == cluster_id)] # dataframe for that cluster
...
def make_combined_figure(figures_path, spatial_firing, sorter_name = settings.sorterName):
    ...
    for cluster_index, cluster_id in enumerate(spatial_firing.cluster_id):
        ...
        if os.path.exists(rate_map_autocorrelogram_path):
            autocorrelogram_10 = mpimg.imread(rate_map_autocorrelogram_path)
        ...
        plt.savefig(figures_path + cluster_df['session_id'].iloc[0] + '_' + str(cluster_id) + '.png', dpi=1000)
        plt.close()
