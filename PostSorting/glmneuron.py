...
def getSpikePopulationXR(spiketrain, cluster_id, Fs):
    ...
    for i, st in enumerate(spiketrain):
        deltaIdx = pd.TimedeltaIndex(st / Fs, unit="s")
        array.loc[deltaIdx, cluster_id[i]] = 1
...
def speed_map_1d(pos, nbins, sampleRate=50, maxSpeed=50, minSpeed=0, removeWrap=True):
    ...
    Keyword Arguments:
        sampleRate {int} -- sampling rate of position signal (default: {50}), used to scale the speed
...
def getVariableIdx(best_model,position_bin = settings.position_bin, 
    speed_bin=settings.speed_bin, accel_bin=settings.accel_bin):
