...
def calculate_firing_rate_for_cluster_parallel(cluster, smooth, firing_data_spatial, positions_x, positions_y, number_of_bins_x, number_of_bins_y, bin_size_pixels, min_dwell, min_dwell_distance_pixels, dt_position_ms):
    ...
    #xy_spikes = xy_spikes[~np.isnan(xy_spikes)]
    ...
    #xy_locs = xy_spikes[~np.isnan(xy_locs)]
    ...
    # bin occupancy
    xedges = np.linspace(0, bin_size_pixels*number_of_bins_x, number_of_bins_x)
    yedges = np.linspace(0, bin_size_pixels*number_of_bins_y, number_of_bins_y)

    occupancy_hist, xedges, yedges = np.histogram2d(positions_x, positions_y, (xedges, yedges))

    occupancy_hist[occupancy_hist<min_dwell] = 0
    occupancy_hist[occupancy_hist!=0] = 1

    firing_rate_map = firing_rate_map*occupancy_hist # occupancy hist is a mask
