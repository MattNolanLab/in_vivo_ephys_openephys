...
def plot_firing_rate_maps_for_trials(recording_folder, spike_data, rewarded, smoothen):
    ...
    for cluster in range(len(spike_data)):
        ...
        ax = avg_spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(bins, avg_probe_spike_rate, '-', color=(31/255, 181/255, 178/255), alpha=0.7)
        ax.fill_between(bins, avg_probe_spike_rate - average_probe_sd,
                        avg_probe_spike_rate + average_probe_sd, facecolor=(31/255, 181/255, 178/255), alpha=0.2)
        ax.plot(bins, avg_beaconed_spike_rate, '-', color='Black')
        ax.fill_between(bins, avg_beaconed_spike_rate - average_beaconed_sd, avg_beaconed_spike_rate + average_beaconed_sd,
        ...
        ax.xaxis.set_ticks_position('bottom')
        plt.subplots_adjust(hspace=.35, wspace=.35, bottom=0.2, left=0.3, right=0.87, top=0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_rate_map_Cluster_' + str(
            cluster_index + 1) + suffix + '_p.png', dpi=200)
        plt.close()
    return
