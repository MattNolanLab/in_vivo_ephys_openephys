...
for i = 2:length(posx);
if isnan(posx(i));
posxnew(i)  = posxnew(i-1);
posynew(i) = posynew(i-1);
else
    posxnew(i) = posx(i);
    posynew(i) = posy(i);
end
dist(i) = abs(pdist2([posxnew(i) posynew(i)],[posxnew(i-1) posynew(i-1)]));
if dist(i) > 200
    posx(i) = NaN;
    posy(i) = NaN;
    posxnew(i) = NaN;
...
% 
% modified by Steven Huang May 2007
% the missing data at the beginning of the position file was changed to the first available position data
% instead of been chopped off.
...
% replace the missing data in the start of session with the first available position data
lastmissing = min(find(isfinite(posx))) - 1;
%posx = posx(lastmissing+1:end);
%posy = posy(lastmissing+1:end);
%post = post(lastmissing+1:end);
n = length(posx);
for ii=1:lastmissing
  posx(ii) = posx(lastmissing+1);
  posy(ii) = posy(lastmissing+1);
end
...
if (sum(missing))
    ...
    x = interp1( post(~missing), posx(~missing), post);
    y = interp1( post(~missing), posy(~missing), post);
    for i = 1:10
