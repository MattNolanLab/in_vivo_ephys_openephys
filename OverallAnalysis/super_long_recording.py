...
def plot_shuffled_number_of_bins_vs_observed(cell):
    ...
    for index, field in cell.iterrows():
        ...
        ax.axvline(x=field.number_of_different_bins_bh, color='navy', linewidth=3)
        ax.xaxis.set_tick_params(labelsize=20)
        ax.yaxis.set_tick_params(labelsize=20)
        # plt.xscale('log')
        max_x_value = max(field.number_of_different_bins_bh, shuffled_distribution.max())
        plt.xlim(0, max_x_value + 1)
        plt.ylabel('N (shuffles)', fontsize=24)
        plt.xlabel('N (significant bins)', fontsize=24)
        plt.tight_layout()
        plt.savefig(analysis_path + str(field.field_id) + 'number_of_significant_bars_shuffled_vs_real_example.png')
        plt.close()
