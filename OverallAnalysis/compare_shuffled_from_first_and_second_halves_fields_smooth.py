...
local_path = OverallAnalysis.folder_path_settings.get_local_path() + '/compare_first_and_second_shuffled_fields_smooth/'
...
def distributive_shuffle(field_data, number_of_bins=20, number_of_times_to_shuffle=1000, smooth=False):
...
def compare_observed_and_shuffled_correlations(iterator, grid_data, all_cells, aggregated_data, sampling_rate_video):
    ...
    first_half = distributive_shuffle(first_half, number_of_bins=360, number_of_times_to_shuffle=1000, smooth=True)
    first_half = OverallAnalysis.shuffle_field_analysis.analyze_shuffled_data(first_half, local_path + '/first/',
                                                                              sampling_rate_video, number_of_bins=360,
                                                                              shuffle_type='distributive', smooth=True, add_stars=False)

    second_half = add_rate_map_values_to_field(second_half_whole_cell, second_half)
    second_half = distributive_shuffle(second_half, number_of_bins=360, number_of_times_to_shuffle=1000, smooth=True)
